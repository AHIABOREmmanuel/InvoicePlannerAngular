...
export class BodyComponent implements OnInit {
  ...
  private calculateStatistics(): void {
    ...
    this.facturesPayees = this.factures.filter(f => f.etat === 'PAYEE').length;
    this.facturesEnAttente = this.factures.filter(f => f.etat === 'EN_ATTENTE' || f.etat === 'ENVOYEE').length;
    this.facturesRetard = this.factures.filter(f => {
      if (f.etat !== 'EN_ATTENTE' && f.etat !== 'ENVOYEE') return false;
    ...
    this.devisEnAttente = this.devis.filter(d => d.statut === 'EN_ATTENTE' || d.etat === 'EN_ATTENTE').length;
  ...
  // Formater les montants avec le symbole de devise
  formatMontant(montant: number): string {
    return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'XOF' }).format(montant || 0);
  }
}
